...
The backend component to remotely control a Foscam IP camera. It uses the [Serverless Application Framework](https://serverless.com/) to implement an AWS Lambda function in TypeScript, deploy it via CloudFormation, and publish it through API Gateway.
...
This project shows example Lambda function implementations with the following layers (see the `src/device` folder):

- **Handler**: The handler is the endpoint that is called by AWS when it executes your Lambda. See `device.ts`.
- **Controller**: The controller is responsible for transforming any operation result to an HTTP response. See `device.controller.ts`.
- **Service**: The service is responsible for implementing the business logic, and provide the operation result. See `device.service.ts`.
- **Repository**: The repository is responsible for providing the data for the service (if necessary).
